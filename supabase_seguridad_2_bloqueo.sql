...
--    - Solo el admin: usuarios, servicios, ofertas, combos, rubros, reglas de reels, horarios de stories, tareas de recepcion.
...
returns boolean language sql stable security definer set search_path = public as $$
  select case
    ...
    when k in ('luffy/users','luffy/servicios','luffy/ofertas','luffy/combos','luffy/rubros',
               'luffy/reglas_reels','luffy/story_horarios','luffy/tareas_recepcion') then false
    else true
  end
$$;
